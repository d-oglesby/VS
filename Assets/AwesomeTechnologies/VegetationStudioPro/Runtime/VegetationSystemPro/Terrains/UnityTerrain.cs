...
//    public struct UnityTerrainSampleConcaveJob : IJob
//    {
//        [ReadOnly]
...
//
//                Vector3 worldPosition = vegetationInstance.Position;
//                Vector3 terrainSpacePositon = worldPosition - TerrainPosition;
//                float2 heightmapPosition = new float2(terrainSpacePositon.x / HeightMapScale.x, terrainSpacePositon.z / HeightMapScale.z);
//
...
//    }
    ...
    public partial class UnityTerrain : MonoBehaviour, IVegetationStudioTerrain
    {
        ...
        void Start()
        {
            if (_terrain && DisableTerrainTreesAndDetails)
            {
        ...
        void LoadHeightData()
        {
            ...
            //{
            //    floatingOriginOffset = VegetationStudioManager.GetFloatingOriginOffset();
            //}
            //_terrainPosition = _terrain.transform.position - floatingOriginOffset;
            ...
            float[,] hs = _terrain.terrainData.GetHeights(0, 0, _heightmapWidth, _heightmapHeight);

            //_nativeManagedFloatArray = new ManagedNativeFloatArray(hs);
            //Heights = (NativeArray<float>)_nativeManagedFloatArray.NativeArray;
        ...
        public Texture2D GetTerrainTexture(int index)
        {
            ...
            if (!_terrain.terrainData) return null;

#if UNITY_2018_3_OR_NEWER
            if (_terrain.terrainData.terrainLayers.Length > index)
            {
                if (_terrain.terrainData.terrainLayers[index])
                {
                    return _terrain.terrainData.terrainLayers[index].diffuseTexture;
                }
                else
                {
                    return null;
...
#else
            if (_terrain.terrainData.splatPrototypes.Length > index)
            {
                return _terrain.terrainData.splatPrototypes[index].texture;
            }
#endif
        ...
        {
            if (!_terrain) return new TerrainLayer[0];
        ...
        {
            if (!_terrain) return new SplatPrototype[0];
            return _terrain.terrainData.splatPrototypes;
        }
        ...
        public void SetSplatPrototypes(SplatPrototype[] splatPrototypes)
        {
            if (_terrain)
            {
                _terrain.terrainData.splatPrototypes = splatPrototypes;
            }
        }
#endif
        ...
        public Bounds TerrainBounds
        {
            get
            {
                if (_terrain)
                {
                    return new Bounds(_terrain.terrainData.bounds.center + TerrainPosition,
